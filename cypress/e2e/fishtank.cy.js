...
Cypress.Commands.add("login", () => {
  cy.session(
    ...
    () => {
      ...
      cy.get(":nth-child(1) > input", { log: false }).type(username, {
        log: false,
      });
      cy.get(":nth-child(2) > input", { log: false }).type(password, {
        log: false,
      });
    },
    {
      validate() {
        cy.document().its("cookie").should("contain", "auth-token");
      },
    },
  );
});
...
it("downloaded all fishtank episodes", () => {
  ...
  cy.visit("https://www.fishtank.live/");
  // Get episode list
  cy.get('[class^="episodes_episodes-grid"]', { timeout: 10000 }).then(
    ([allVideos]) => {
      const [, reactFiber] = Object.entries(allVideos).find(([key]) =>
        key.startsWith("__reactFiber"),
      );
      cy.wrap(
        reactFiber.memoizedProps.children
          .map((x) => x.props.episode)
          .filter((x) => x.jwt)
          .map((x, i) => [
            {
